...
import { LanguageModelV2, EmbeddingModelV2, NoSuchModelError } from '@ai-sdk/provider';
...
describe('BasetenProvider', () => {
  ...
  describe('chatModel', () => {
    it('should construct a chat model with correct configuration for default Models API', () => {
      const provider = createBaseten();
    ...
    it('should handle /sync/v1 endpoints correctly', () => {
      ...
      expect(url).toBe('https://model-123.api.baseten.co/environments/production/sync/v1/chat/completions');
    });

    it('should handle /predict endpoints with request transformation', () => {
      const provider = createBaseten({
      ...
      const config = constructorCall[1];
      const url = config.url({ path: '/chat/completions' });
      expect(url).toBe('https://model-123.api.baseten.co/environments/production/predict');
    });
  });
  ...
  describe('textEmbeddingModel', () => {
    it('should throw error when no modelURL is provided', () => {
      ...
      }).toThrow('No model URL provided for embeddings. Please set modelURL option for embeddings.');
    });
    ...
    it('should construct embedding model for /sync/v1 endpoints', () => {
      ...
      expect(url).toBe('https://model-123.api.baseten.co/environments/production/sync/v1/embeddings');
    });
