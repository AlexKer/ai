...
const hasApiKey = process.env.BASETEN_API_KEY && process.env.BASETEN_API_KEY !== '';

describe('BasetenProvider Integration Tests', () => {
  beforeAll(() => {
    if (!hasApiKey) {
      console.warn('⚠️  BASETEN_API_KEY not found. Skipping integration tests.');
    }
  });

  describe('Default Models API', () => {
    it('should generate text with default Models API', async () => {
      if (!hasApiKey) {
